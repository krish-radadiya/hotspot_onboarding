# Hotspot Onboarding Assignment

## Overview
This project implements the Flutter onboarding questionnaire for Hotspot Hosts.

## Features Implemented
- Experience Selection screen with animated cards and comment box
- Onboarding Question screen with text, audio, and video recording
- Fully responsive design with keyboard-safe layouts
- Waveform animation for audio
- Integrated state management with Bloc
- Video and audio playback with delete options
- Animated transitions and UI consistency

## Brownie Points
- Pixel-perfect UI from Figma
- Advanced animations (card scaling, transitions)
- Responsive layout on all devices
- State management with Bloc
- Optional: Used Dio for API calls (if you did)

## Demo Video
(Attach screen recording link here)

## Setup
1. Clone repo
2. Run `flutter pub get`
3. Run on emulator/device


https://github.com/user-attachments/assets/4bcc1c1c-c585-4149-aa3d-850b63b3dfaa


<img src="https://github.com/user-attachments/assets/2b34174b-9124-4d4c-a917-516282d0f12f" width="250">
<img src="https://github.com/user-attachments/assets/55f7d449-3744-4272-a77f-4ac16e94e902" width="250">
<img src="https://github.com/user-attachments/assets/e4ef0adc-b910-4aeb-a5c8-a050b4e9d871" width="250">
<img src="https://github.com/user-attachments/assets/71d426ba-e81c-447d-be02-19c7995599b2" width="250">
<img src="https://github.com/user-attachments/assets/7502913f-4140-4529-9dfb-eab0a1d6d3f2" width="250">
<img src="https://github.com/user-attachments/assets/9d171698-620e-4ebc-aeb0-2461137f3a75" width="250">
<img src="https://github.com/user-attachments/assets/11309fd4-24dc-4b45-b3ac-4f93d8c9e9cb" width="250">







